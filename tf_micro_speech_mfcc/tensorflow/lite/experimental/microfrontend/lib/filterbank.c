...
static int is_first_bank = 1;

static FILE *fp_spectro;
static FILE *fp_energy;
static FILE *fp_accum;
static FILE *fp_filtersqrt;
...
void FilterbankConvertFftComplexToEnergy(struct FilterbankState* state,
  ...
  if (is_first)
  {
    fp_energy = fopen("energies.txt", "w");
    fp_spectro = fopen("fft_out.txt", "w");
    is_first = 0;
  }
  ...
  //printf ("Energy start_index: %d\n", state->start_index); // 9
  //printf ("Energy end_index: %d\n", end_index); // 225
  for (i = state->start_index; i < end_index; ++i) {
    ...
    *energy++ = mag_squared;
    ...
    fprintf(fp_spectro, "%d %d | ", real, imag);
    fprintf (fp_energy, "%d ", *energy);
...
uint32_t* FilterbankSqrt(struct FilterbankState* state, int scale_down_shift) {
  ...
  {
    is_first_bank = 0;
    fp_accum = fopen("filterbank_accum.txt", "w");
    fp_filtersqrt = fopen("filterbank_sqrt.txt", "w");
  }
  ...
  for (i = 0; i < num_channels; ++i) {
    *output++ = Sqrt64(*work++) >> scale_down_shift;
    fprintf (fp_accum, "%ld ", *work);
    fprintf (fp_filtersqrt, "%u ", *output);
  }
  fprintf (fp_accum, "\n\n");
  fprintf (fp_filtersqrt, "\n\n");
