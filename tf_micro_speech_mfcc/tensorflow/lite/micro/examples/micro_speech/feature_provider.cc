...
TfLiteStatus FeatureProvider::PopulateFeatureData(
  ...
  *how_many_new_slices = slices_needed;
  ...
  printf ("slices_needed: %d\n", slices_needed);
  printf ("slices_to_keep: %d\n", slices_to_keep);
  printf ("slices_to_drop: %d\n", slices_to_drop);
  ...
  if (slices_needed > 0) {
    //std::cout << ("Loop for going through slices\n") << std::endl;
    for (int new_slice = slices_to_keep; new_slice < kFeatureSliceCount;
      ...
      printf ("new_step: %d\n", new_step);
      const int32_t slice_start_ms = (new_step * kFeatureSliceStrideMs); // -64
      printf ("slice_start_ms: %d\n", slice_start_ms);
